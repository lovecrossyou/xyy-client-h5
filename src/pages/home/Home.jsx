...
import { mapStateToProps, mapActionsToProps } from './index';
...
export default class Home extends React.Component {
  ...
  render() {
    ...
    return (<div className={styles.root}>
      ...
      {init ? (<Scroll {...scrollProps} ref={c => this.scroll = c}>
        <Slide>
          {banner.map(v => (<img key={v.id} src={v.image_url} alt="" />))}
        </Slide>
        ...
        <TitleBar title="推荐商家" />
        {shoplist.map((shop, i) => (<ShopListRow handleClick={() => this.handleRowClick(shop)} key={`${shop.id}--${i}--${new Date().getTime()}`} data={shop} refresh={this.refreshScroll} />))}
      </Scroll>
